...
/// One compiled rule, serialized to a `[[rules]]` table. Field order is the
/// emitted order. `argsPattern` is a real TOML string (NOT a hand-quoted literal)
/// so the `toml` serializer escapes patterns containing quotes/backslashes — a
/// pattern like `tr -d "'"` carries a single quote that a `'…'` literal would
/// terminate early, producing invalid TOML that breaks Gemini's policy load.
    ...
    let policy = GeminiPolicyFile {
        rules: arr
            ...
            .map(|rule| GeminiPolicyRule {
                ...
                priority: rule["priority"].as_f64(),
            })
            .collect(),
    };
    // toml::to_string emits `[[rules]]` array-of-tables and escapes every string
...
"#,
    ...
    fn testSerializeGeminiPolicyTomlEscapesQuotesAndBackslashes() {
        ...
        assert_eq!(
            parsed["rules"][0]["argsPattern"].as_str(),
            Some(pattern),
