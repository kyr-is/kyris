...
/// Well-known agent hook/plugin/policy files that kyris install creates, across
/// every supported agent and including legacy locations. This is the
/// manifest-independent backstop that runs even when an agent's profile is gone
/// (so `undo_agent`'s per-agent scrub never ran for it). Each is removed only
/// when it carries a kyris marker ([`crate::agents::scrub::file_has_kyris_marker`]).
///
...
pub(crate) const WELL_KNOWN_HOOK_PATHS: &[&str] = &[
    ...
    // codex (legacy + current)
    ".codex/kyris_pretooluse.sh",
    ".codex/hooks/agentpact_pretooluse.sh",
    // gemini (legacy + current + compiled policy)
    ".gemini/kyris_pretooluse.sh",
    ".gemini/hooks/agentpact_beforetool.sh",
    ".gemini/policies/agentpact.toml",
    // cline
    ".cline/hooks/PreToolUse.cjs",
    // opencode plugin bridge
    ".config/opencode/kyris-governance.js",
];
...
fn sweep_one(path: &Path, log: &InstallLog) -> Result<(), String> {
    ...
    if !crate::agents::scrub::file_has_kyris_marker(path) {
