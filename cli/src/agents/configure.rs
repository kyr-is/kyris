...
/// plugin-hook agent — only the agent id is substituted.
pub(super) fn plugin_hook_source(agent_id: &str) -> String {
...
export const KyrisGovernance = async (input) => ({
  "tool.execute.before": async (inp, out) => {
    ...
    // (opencode has no launch-dir env var, so it must ride in the payload).
    const payload = JSON.stringify({ tool_name: inp.tool, tool_input: out?.args ?? {}, cwd: input.directory })
    const res = spawnSync(KYRIS_BIN, ["hook", "check", "--agent", "__AGENT_ID__"], {
      input: payload,
      cwd: input.directory,
      encoding: "utf8",
      timeout: 600000,
    ...
    // Fail OPEN if the binary can't run at all (not found / timeout) — matches
    // the shell adapter's "kyris not found → exit 0 → ungoverned" posture.
    if (res.error) return
