...
fn reconcile_agent(agent: &dyn AgentDescriptor) -> Result<AgentProfile, String> {
    ...
    for existing_fp in &profile.managed_files {
        ...
        if current_hash != existing_fp.content_hash && !file_contains_marker(path, markers) {
            // Agent was reinstalled — content markers are gone.
            // The apply module will re-inject on next `kyris agents setup`.
            // For now, mark surfaces as needing repair by resetting to probe state.
            // (The actual re-injection happens in apply.rs, called by the CLI.)
        }
    }
    ...
    // If native was observed, promote surfaces that were adapted.
    if profile.last_native_seen.is_some() {
        if profile.execution.level == CapLevel::Adapted {
            profile.execution = super::profile::SurfaceState::native();
        }
        if profile.tool.level == CapLevel::Adapted {
